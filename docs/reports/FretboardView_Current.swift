...
struct FretboardView: View {
    ...
    private func calculateRowHeight(availableHeight: CGFloat) -> CGFloat {
        // In landscape, maximize space usage to fit all 6 strings
        if isLandscape {
            let usableHeight = availableHeight - dynamicTopBarHeight - 5  // Minimal margin
            let calculatedHeight = usableHeight / CGFloat(strings.count)
            return calculatedHeight  // Use calculated height to fit all strings
        } else {
