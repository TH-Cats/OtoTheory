...
* **Lite（Web／iOS Free）**
  ...
  * 12コード上限／プリセット20／自動ループ（4/4・BPM）／**Undoなし**／Sketch 3件（ローカル）。
...
### 1.3 Melody/Solo Analyze（録音→Key/Scale）
...
* 保存単位は **Sketch（ソングスケッチ）**。**進行＋Key/Scale＋Capo（Shaped）＋Fretboard表示** を一括保存。
...
* **Free（Web/iOS）**：Find Chords 拡張／二層 Overlay／Capo Top2／Progression Lite（12上限・プリセット・自動ループ）／Sketch 3 件／PNG・テキスト出力。
* **Pro（iOS）**：**セクション編集**／**MIDI 出力**／**Sketch無制限**／将来の高度機能。
