...
* **Chord Library 機能**：
  - **フォーム表示**：
    - **v3.1 現状（Static v0, iOS）**：Open / Root‑6 / Root‑5 / Root‑4（Triadは今後）
    - **表示順を固定**：Open → Root‑6 → Root‑5 → Root‑4 → Triad‑1 → Triad‑2
    - **タイトル推定**：`shapeName`が未指定のフォームはUIで名称を推定（開放弦/バレー有無）
    - **将来拡張（v3.1.1+）**：Compact（上4弦軽量）/ Color（add9, 6/9, sus2/4等）
      - **Compact**：上4弦中心の把弦負荷が軽い実用ボイシング
      - **Color**：add9 / 6/9 / sus2/4 等から**響き価値が高い実戦型**を精選
        - プレビュー自由／進行追加は既存のPro制限に従う
