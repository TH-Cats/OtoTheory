...
## 1. デプロイ手順（Deployment Process）

### 1.1 Web版デプロイ（GitHub Actions + Vercel）

**現在のデプロイ構成**:
- **リポジトリ**: GitHub (TH-Cats/OtoTheory)
- **自動デプロイ**: GitHub Actions + Vercel
- **トリガー**: `main`ブランチへのプッシュ（`ototheory-web/**`パスの変更時）

**デプロイフロー**:
```
コード修正 → git commit → git push origin main → GitHub Actions → Vercel自動デプロイ
```
...
3. **自動デプロイ確認**
   - GitHub Actions: https://github.com/TH-Cats/OtoTheory/actions
   - Vercel Dashboard: https://vercel.com/dashboard
   - 本番サイト: https://www.ototheory.com
...
**デプロイ時間**: 通常3-5分（ビルド2分 + デプロイ1-3分）

### 1.2 iOS版デプロイ（TestFlight）
...
**詳細手順**: `docs/TestFlight_Setup_Guide.md`を参照

### 1.3 トラブルシューティング

**Web版デプロイエラー**:
- **ビルドエラー**: `npm run build`でローカル確認
- **GitHub Actions失敗**: https://github.com/TH-Cats/OtoTheory/actions でログ確認
- **Vercelデプロイ失敗**: Vercel Dashboardでログ確認
- **Secrets設定**: GitHub Settings → Secrets and variables → Actions

**iOS版デプロイエラー**:
- **Archive失敗**: Code Signing設定確認
- **Upload失敗**: Provisioning Profile更新
- **TestFlight設定**: App Store ConnectでBuild状態確認

**緊急時の手動デプロイ**:
```bash
# Vercel CLI使用（GitHub Actionsが失敗した場合）
cd /Users/nh/App/OtoTheory/ototheory-web
npx vercel --prod --yes
```
...
## 2. データ管理（Single Source of Truth）
