...
//  OtoTheory
//
//  Scale master data based on OtoTheory_Scales_v3.csv
//  Single Source of Truth for all scale definitions
//

import Foundation

// MARK: - Scale Master Data Structures

struct ScaleMasterItem {
    let id: String
    ...
    let tones: Int
    let heptatonic: Bool
    let romanException: Bool
    let comments: ScaleComments
}

struct ScaleComments {
    let ja: CommentSections
    let en: CommentSections
}

struct CommentSections {
    let vibe: String    // 雰囲気 (Vibe)
    let use: String     // 特徴 (Usage)
    let `try`: String   // Try
    let theory: String  // 理論 (Theory)
}

// MARK: - Scale Master Data

struct ScaleMaster {
    static let allScales: [ScaleMasterItem] = [
        // Basic
        ScaleMasterItem(
            id: "major",
            categoryEn: "Basic",
            ...
            tones: 7,
            heptatonic: true,
            romanException: false,
            comments: ScaleComments(
                ja: CommentSections(
    ...
    static func scaleById(_ id: String) -> ScaleMasterItem? {
        return allScales.first { $0.id == id }
    }
    
    static func scalesByCategory(_ category: String) -> [ScaleMasterItem] {
        return allScales.filter { $0.categoryEn == category }
    }
    
    static func allCategories() -> [String] {
        return Array(Set(allScales.map { $0.categoryEn })).sorted()
    }
    
    static func scalesForMode(_ mode: String) -> [ScaleMasterItem] {
        switch mode.lowercased() {
        case "major":
            return allScales.filter { scale in
                scale.categoryEn == "Basic" || scale.categoryEn == "Modes" || 
                scale.categoryEn == "Pentatonic & Blues" || scale.categoryEn == "Advanced"
            }
        case "minor":
            return allScales.filter { scale in
                scale.categoryEn == "Basic" || scale.categoryEn == "Modes" || 
                scale.categoryEn == "Pentatonic & Blues" || scale.categoryEn == "Minor family" || 
                scale.categoryEn == "Advanced"
            }
        default:
            return allScales
        }
