...
const WEB_OUTPUT = path.join(__dirname, '../src/lib/articlesData.ts');
  ...
  for (const lang of languages) {
    ...
    try {
      ...
      for (const file of mdFiles) {
        ...
        // 日付を文字列として保持（ISO形式をYYYY-MM-DDに変換）
        let updatedDate = frontmatter.updated || new Date().toISOString().split('T')[0];
        if (typeof updatedDate === 'string' && updatedDate.includes('T')) {
