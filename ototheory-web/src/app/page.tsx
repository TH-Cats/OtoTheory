...
export default function Home() {
  return (
    <div className="flex flex-col gap-8">
      <section className="rounded-lg p-6 text-white" style={{background: 'linear-gradient(90deg, var(--brand-primary), var(--brand-secondary))'}}>
        <h1 className="text-2xl font-semibold">OtoTheory</h1>
        <p className="opacity-90">Use Theory Without Tears</p>
      </section>
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <Link href="/progression" className="rounded-lg p-6 border hover:bg-black/5 dark:hover:bg-white/5">
          <h2 className="font-semibold mb-1">Progression</h2>
          <p className="text-sm opacity-80">Build chord progressions</p>
        </Link>
        <Link href="/analyze" className="rounded-lg p-6 border hover:bg-black/5 dark:hover:bg-white/5">
          <h2 className="font-semibold mb-1">Analyze</h2>
          <p className="text-sm opacity-80">Chords / Record / Key&Scale → Result</p>
        </Link>
