...
export interface AnalyzeData {
  keyCandidates?: { keyPc: number; mode: 'major' | 'minor'; confidence: number; why?: string }[];
  [key: string]: any;
}
...
function isHeptatonic(scaleId: string): boolean {
  ...
  return (SCALE_CATALOG.find((s: any) => s.id === scaleId)?.degrees?.length ?? 7) === 7;
}
  ...
  return (
    <section className="ot-card">
      ...
      <div className="ot-section">
        ...
        <div className="ot-block">
          ...
          <Fretboard
            overlay={{
              ...
              chordNotes: chord?.notes?.map(n => PITCHES12[n] || 'C'),
              showScaleGhost: true,
            }}
          />
        ...
        {/* CapoFold */}
        <CapoFold keySig={selectedKey} progression={[]} />
      </div>
    </section>
  );
}
